...
function normalizePayload(raw: any) {
  ...
  const customerEmail = raw.customerEmail || customer.email || ''
  ...
  const meta         = raw.meta   || {}
  const vatRate      = raw.vatRate != null
                   ? Number(raw.vatRate)
                   : undefined
...
// Count rows in `usage` table since a given time, with fallback on column name
...
export async function POST(req: Request) {
  ...
  try {
    ...
    // 4) Parse JSON + normalize
    stage = 'parse.json'
    let body: any
    try { body = await req.json() }
    catch { return json({ ok: false, stage, error: 'Invalid JSON' }, 400) }
    ...
    let   n8nJson = null
    try { n8nJson = text ? JSON.parse(text) : null }
    catch {}
    if (!n8nRes.ok) {
      return json({ ok: false, stage, error: `n8n ${n8nRes.status}`, detail: text }, 502)
    }

    // 9) Persist invoice record for history (now including pdf_url)
